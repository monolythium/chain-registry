...
| `network_registry_sha256` | hex string (64 chars, no `0x`) | no | SHA-256 of the exact mono-core `network-registry.toml` bytes installed on the fleet and bound into genesis. Posture-C release contracts require this pin so public registry metadata, release attestation, node bootstrap policy, and the genesis commitment all identify the same network-registry artifact. |
